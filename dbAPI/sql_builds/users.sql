...
DO $$
BEGIN PERFORM create_trigger_snapshot('users','dir'); END; 
$$ LANGUAGE PLPGSQL;
...
CREATE OR REPLACE FUNCTION users.exists(
	IN	user_id_in		INT
...
CASE
	WHEN (select TRUE from users.dir where user_id = user_id_in) THEN
		user_exists := TRUE;
	ELSE
		user_exists := FALSE;
END CASE;
